/***************************************************************************************************
FireweedLiveFuelMoistureGSI.cpp
 ...
 *
 * @param tempCMin Minimum daily temperature (degrees Celsius).
 * @param vpdPa Mean daily vapor pressure (VPD) deficit (Pa).  The documentation mentions maximum
 *              daily VPD (VPD_max, not to be confused with the VPD_max parameter in the
 *              calculation) can also be used but it is unclear if there are any changes in the
 *              calculation in this case.
 * @param dayLength Daylight length or photoperiod (seconds).
 * 
...
{
	double iT_Min, iVPD, iPhoto;//Components.
	double gsi;//Return value.
	...
	const double T_MMin = -2;//C
	const double T_MMax = 5;//C
	const double VPD_Min = 900;//Pa
	const double VPD_Max = 4100;//Pa
	const double Photo_Min = 36000;//Seconds = 10 hours
	const double Photo_Max = 39600;//Seconds = 11 hours
	...
	{
		Warning("Very unlikely VPD value: " + std::to_string(vpdPa));
	}
	...
	{
		Stop("Invalid day length: " + std::to_string(dayLength));
	}
 ...
 * @param lfmMin Minimum live fuel moisture parameter.
 * @param lfmMax Maximum live fuel moisture parameter.
 * @param gu GSI greenup threshold, defaults to 0.5 (unitless index value).
 * 
 * @returns Percent live fuel moisture (water weight/dry fuel weight * 100%).
 */
double GSI_LiveFuelMoisture(double gsi, double lfmMin, double lfmMax, double gu)
{
	double lfm;//Return value.

	//Validity checking:
	if (gsi < 0.0 || gsi > 1.0)
	{
		Stop("Invalid GSI value: " + std::to_string(gsi));
	}
	if (gu < 0.0 || gu > 1.0)//Values at the edges are unlikley too.
	{
		Stop("Invalid greenup threshold value: " + std::to_string(gu));
	}

	double m = (lfmMax - lfmMin) / (1.0 - gu);//Slope
	double b = lfmMax - m;//Intercept
